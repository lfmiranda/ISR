...
class Schemes {
     ...
     * @return The weight.
     */
    static double findWeight(Instance inst, ParametersManager params) {
        ...
        switch (scheme) {
            case "proximity-x":
            case "proximity-xy":
                weight = proximity(inst, params, p, numDim);
                break;
            case "surrounding-x":
            case "surrounding-xy":
                weight = surrounding(inst, params, p, numDim);
                break;
        }
     ...
     */
    private static double proximity(Instance inst, ParametersManager params, double[] p, int numDim) {
        String scheme = params.getScheme();
        ...
        for (Instance neighbor : inst.getNeighbors()) {
            ...
            sum += Utils.measureDistance(p, q, numDim, params.getDistMetric());
        }
     ...
     */
    private static double surrounding(Instance inst, ParametersManager params, double[] p, int numDim) {
        String scheme = params.getScheme();
        ...
        return Utils.measureDistance(origin, resultant, numDim, params.getDistMetric());
    }
